...
const SOURCE_DIR = path.join(ASSETS_DIR, 'projectiles', 'arrow-source');
const TARGET_PATH = path.join(ASSETS_DIR, 'projectiles', 'arrow.png');
...
console.log('🔄 Processing arrow projectile...\n');
...
if (files.length === 0) {
  ...
  console.log('📝 Place your arrow image in the folder first');
  process.exit(1);
}
...
if (fs.existsSync(TARGET_PATH)) {
  const backupPath = path.join(ASSETS_DIR, 'projectiles', 'arrow.png.backup');
...
if (resizeImage(sourcePath, TARGET_PATH, TARGET_WIDTH, TARGET_HEIGHT)) {
  console.log(`✅ Created arrow.png (${TARGET_WIDTH}x${TARGET_HEIGHT})`);
