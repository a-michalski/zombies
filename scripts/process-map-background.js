...
const SOURCE_DIR = path.join(ASSETS_DIR, 'map', 'background-source');
const TARGET_PATH = path.join(ASSETS_DIR, 'map', 'background.png');
...
console.log('🔄 Processing map background...\n');
...
if (fs.existsSync(TARGET_PATH)) {
  const backupPath = path.join(ASSETS_DIR, 'map', 'background.png.backup');
...
if (resizeImage(sourcePath, TARGET_PATH, TARGET_WIDTH, TARGET_HEIGHT)) {
  console.log(`✅ Created background.png (${TARGET_WIDTH}x${TARGET_HEIGHT})`);
