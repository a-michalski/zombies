...
const SOURCE_DIR = path.join(ASSETS_DIR, 'map', 'path-texture-source');
const TARGET_PATH = path.join(ASSETS_DIR, 'map', 'path-texture.png');
...
console.log('🔄 Processing path texture...\n');
...
if (files.length === 0) {
  ...
  console.log('📝 Place your path texture image in the folder first');
  process.exit(1);
}
...
if (fs.existsSync(TARGET_PATH)) {
  const backupPath = path.join(ASSETS_DIR, 'map', 'path-texture.png.backup');
...
if (resizeImage(sourcePath, TARGET_PATH, TARGET_SIZE, TARGET_SIZE)) {
  console.log(`✅ Created path-texture.png (${TARGET_SIZE}x${TARGET_SIZE})`);
