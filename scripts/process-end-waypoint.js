...
const SOURCE_DIR = path.join(ASSETS_DIR, 'map', 'end-waypoint');
const TARGET_PATH = path.join(ASSETS_DIR, 'map', 'end-waypoint.png');
...
console.log('🔄 Processing end waypoint (ship wreck)...\n');
...
if (files.length === 0) {
  ...
  console.log('📝 Place your end waypoint image in the folder first');
  process.exit(1);
}
...
if (fs.existsSync(TARGET_PATH)) {
  const backupPath = path.join(ASSETS_DIR, 'map', 'end-waypoint.png.backup');
...
if (resizeImage(sourcePath, TARGET_PATH, TARGET_SIZE, TARGET_SIZE)) {
  console.log(`✅ Created end-waypoint.png (${TARGET_SIZE}x${TARGET_SIZE})`);
