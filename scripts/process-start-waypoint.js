...
const SOURCE_DIR = path.join(ASSETS_DIR, 'map', 'start-waypoint');
const TARGET_PATH = path.join(ASSETS_DIR, 'map', 'start-waypoint.png');
...
console.log('🔄 Processing start waypoint (abyss/chasm)...\n');
...
if (files.length === 0) {
  ...
  console.log('📝 Place your start waypoint image in the folder first');
  process.exit(1);
}
...
if (fs.existsSync(TARGET_PATH)) {
  const backupPath = path.join(ASSETS_DIR, 'map', 'start-waypoint.png.backup');
...
if (resizeImage(sourcePath, TARGET_PATH, TARGET_SIZE, TARGET_SIZE)) {
  console.log(`✅ Created start-waypoint.png (${TARGET_SIZE}x${TARGET_SIZE})`);
